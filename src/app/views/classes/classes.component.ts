...
export class ClassesComponent implements OnInit {
  ...
  getPackages () {
    ...
    this.eventsService.getAllClassesPaging(this.currentPage, this.itemsPerPage).subscribe(
      (responseBuilder) => {
            ...
            forkJoin([this.eventClassTypeService.getAllEventClassTypes(),
              ...
              .subscribe(responses => {
                if (responses[0].code === + this.apiConfig.SUCCESS &&
                  ...
                  this.loader.close();
                  const title = isNew ? 'Add new class' : 'Update class';
                  const dialogRef: MatDialogRef<any> = this.dialog.open(NgxClassesPopupComponent, {
                    width: '720px',
                    height: '500px',
                    disableClose: true,
                    data: {
    ...
    this.eventsService.getEvent(data.id).subscribe(
      (responseBuilder) => {
        if (responseBuilder.code === + this.apiConfig.SUCCESS) {
          ...
          const dialogRef: MatDialogRef<any> = this.dialog.open(NgxClassImagesPopupComponent, {
            disableClose: true,
            data: {
              title: title,
              payload: data,
